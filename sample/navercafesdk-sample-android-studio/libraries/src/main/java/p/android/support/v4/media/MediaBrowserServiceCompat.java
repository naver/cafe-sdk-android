...
import p.android.support.annotation.IntDef;
import p.android.support.annotation.NonNull;
import p.android.support.annotation.Nullable;
import p.android.support.v4.app.BundleCompat;
import p.android.support.v4.media.session.MediaSessionCompat;
import p.android.support.v4.os.ResultReceiver;
import p.android.support.v4.util.ArrayMap;
...
public abstract class MediaBrowserServiceCompat extends Service {
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MediaBrowserProtocol.CLIENT_MSG_CONNECT:
                    mServiceImpl.connect(data.getString(MediaBrowserProtocol.DATA_PACKAGE_NAME),
                            data.getInt(MediaBrowserProtocol.DATA_CALLING_UID), data.getBundle(MediaBrowserProtocol.DATA_ROOT_HINTS),
                            new ServiceCallbacksCompat(msg.replyTo));
                    break;
                case MediaBrowserProtocol.CLIENT_MSG_DISCONNECT:
                    mServiceImpl.disconnect(new ServiceCallbacksCompat(msg.replyTo));
                    break;
                case MediaBrowserProtocol.CLIENT_MSG_ADD_SUBSCRIPTION:
                    mServiceImpl.addSubscription(data.getString(MediaBrowserProtocol.DATA_MEDIA_ITEM_ID),
                            data.getBundle(MediaBrowserProtocol.DATA_OPTIONS), new ServiceCallbacksCompat(msg.replyTo));
                    break;
                case MediaBrowserProtocol.CLIENT_MSG_REMOVE_SUBSCRIPTION:
                    mServiceImpl.removeSubscription(data.getString(MediaBrowserProtocol.DATA_MEDIA_ITEM_ID),
                            data.getBundle(MediaBrowserProtocol.DATA_OPTIONS), new ServiceCallbacksCompat(msg.replyTo));
                    break;
                case MediaBrowserProtocol.CLIENT_MSG_GET_MEDIA_ITEM:
                    mServiceImpl.getMediaItem(data.getString(MediaBrowserProtocol.DATA_MEDIA_ITEM_ID),
                            (ResultReceiver) data.getParcelable(MediaBrowserProtocol.DATA_RESULT_RECEIVER));
                    break;
                case MediaBrowserProtocol.CLIENT_MSG_REGISTER_CALLBACK_MESSENGER:
                    mServiceImpl.registerCallbacks(new ServiceCallbacksCompat(msg.replyTo));
                    break;
                case MediaBrowserProtocol.CLIENT_MSG_UNREGISTER_CALLBACK_MESSENGER:
    ...
    private class ServiceCallbacksCompat implements ServiceCallbacks {
        ...
        public void onConnect(String root, MediaSessionCompat.Token session, Bundle extras)
            ...
            extras.putInt(MediaBrowserProtocol.EXTRA_SERVICE_VERSION, MediaBrowserProtocol.SERVICE_VERSION_CURRENT);
            ...
            sendRequest(MediaBrowserProtocol.SERVICE_MSG_ON_CONNECT, data);
        }

        public void onConnectFailed() throws RemoteException {
            sendRequest(MediaBrowserProtocol.SERVICE_MSG_ON_CONNECT_FAILED, null);
        }
        ...
        public void onLoadChildren(String mediaId, List<MediaBrowserCompat.MediaItem> list,
            ...
            if (list != null) {
                data.putParcelableArrayList(MediaBrowserProtocol.DATA_MEDIA_ITEM_LIST,
                        list instanceof ArrayList ? (ArrayList) list : new ArrayList<>(list));
            }
            sendRequest(MediaBrowserProtocol.SERVICE_MSG_ON_LOAD_CHILDREN, data);
        }

        private void sendRequest(int what, Bundle data) throws RemoteException {
            Message msg = Message.obtain();
            msg.what = what;
            msg.arg1 = MediaBrowserProtocol.SERVICE_VERSION_CURRENT;
            msg.setData(data);
            mCallbacks.send(msg);
        }
    }
    ...
    private class ServiceCallbacksApi21 implements ServiceCallbacks {
        ...
        public void onConnect(String root, MediaSessionCompat.Token session, Bundle extras)
            ...
            mMessenger = new Messenger(mHandler);
            BundleCompat.putBinder(extras, MediaBrowserProtocol.EXTRA_MESSENGER_BINDER, mMessenger.getBinder());
            extras.putInt(MediaBrowserProtocol.EXTRA_SERVICE_VERSION, MediaBrowserProtocol.SERVICE_VERSION_CURRENT);
    ...
    BrowserRoot onGetRoot(@NonNull String clientPackageName,
